...
-- If the first parameter is set as a boolean, it will choose between Undertale and Deltarune Yellow Soul mode (second parameter will be the angle)
-- If the first parameter is a number and the second parameter is empty, it will be the angle of the soul
function YellowSoul:init(x, y, undertale, angle)
    ...
    self.undertale = undertale -- whether to use undertale yellow soul (false by default)
    ...
    self.charge_speed = 2 -- how fast it takes to charge up a big-shot
    self.allow_cheat = Kristal.getLibConfig("magical-glass", "yellow_soul_allowcheat") -- whether the player is allowed to chain bigshots rapidly by pressing two different "confirm" buttons simultaneously

    -- internal variables
    self.hold_timer = 0
    self.charge_sfx = nil
    self.shot_timer = 0
    self.undertale_shot_timer = 0

    -- Since the soul has custom rotation, this will prevent using the monster soul sprites
...
function YellowSoul:update()
    ...
    -- cancel all special soul traits when not defending
    ...
    self.undertale_shot_timer = MathUtils.approach(self.undertale_shot_timer, 0, DT)
...
function YellowSoul:getChargeSpeed() return self.charge_speed end
...
function YellowSoul:fireShot(big)
    ...
    if big then -- bigshot
        ...
        self.shot_timer = 5 -- delay normal shots after a bigshot for 5 frames
    elseif self.undertale then
        if #Game.stage:getObjects(YellowSoulShotUndertale) > 0 and self.undertale_shot_timer > 0 then return end -- only allow 1 at once or after half a second
        local radius = 2
        ...
        Assets.playSound("yellowsoul/heartshot_undertale")
        self.undertale_shot_timer = 0.5 -- half a second
    else
