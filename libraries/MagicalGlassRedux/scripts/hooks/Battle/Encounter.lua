    ...
    elseif Game:isLight() then
        ...
        for _, member in ipairs(Game.battle.party) do
            ...
            if lv ~= member.chara:getLightLV() then
                win_text = string.format("* You won!\n* Got %s EXP and %s %s.\n* Your LOVE increased.", xp, money, Game:getConfig("lightCurrency"):lower())
...
function Encounter:beforeStateChange(old, new, reason)
    -- Used to prevent gaining dark money or getting stronger (the dark world variation) in the light world
    if new == "VICTORY" then
        if Game:isLight() then
