...
function LightInventory:tryGiveItem(item, ignore_dark)
    if ignore_dark or item.light then
        local success, message = Inventory.tryGiveItem(self, item, ignore_dark)
        if not success then
            message = "* (You have too many [color:yellow]ITEMs[color:reset] to take [color:yellow]" .. item:getName() .. "[color:reset].)"
        end
